...
export default async (currentUser = {}) => {
  if (isHttps || document.location.hostname === 'localhost') {
    ...
    if ('Notification' in window) {
      await Notification.requestPermission().then(async (permission) => {
        if (permission === 'granted') {
          ...
          getToken(messaging, {
            vapidKey:
              'BL3b29Ff08PfKy_LrFvFfrrYDH3dXFc9Sb6BTuefXyo4p_kvYJHSWwvhy_9kwGMSxsYXM1DUSujjh_dS_445-Ig',
          })
            .then(async (currentToken) => {
              if (currentToken) {
                ...
                message.info({
                  content: 'No registration token available. Request permission to generate one.',
                });
                // ...
              }
            })
      ...
      // })
    } else alert('This browser does not support desktop notification');
  }
};
